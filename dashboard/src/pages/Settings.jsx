...
// Worker base URL for the Connect Instagram section. Same environment rule as
// Inbox.jsx: staging dashboard hosts hit the staging Worker, everything else
// (production pages.dev, custom domains, local dev) hits production. Kept as a
// duplicate const rather than a shared import to match the existing pattern in
// this codebase; if a third page needs it, lift all three into lib/ at once.
...
export default function Settings() {
  ...
  // Instagram connection status, read from the Worker's /meta/connection-status.
  ...
  // Separate from load() on purpose. This is a Worker round trip, not a Supabase
  // one, and it must not hold up the rest of the page: the settings form renders
  // while the connection status is still resolving.
  ...
  return (
    <div className="page">
      ...
      {/* First card on the page on purpose. It is the first thing a new client
          (and a Meta App Review reviewer following "use Connect Instagram in
          Settings") needs to find, and it gates everything below it: no
          connected account means no inbound DMs and nothing to review. */}
      <div className="card">
        <div style={{ marginBottom: '14px' }}>
          <div className="card-title" style={{ marginBottom: '4px' }}>Instagram Connection</div>
