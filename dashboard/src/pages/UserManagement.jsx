...
const ROLE_GUIDE = [
  ...
  { role: 'Admin', badge: 'badge-green', desc: 'Full platform access. Can manage clients and setters but cannot edit or remove other admins.' },
  { role: 'Client', badge: 'badge-blue', desc: 'Access is limited to their assigned bot. Default access includes Inbox, Bot Tester, and Analytics. Permissions can be customised.' },
  { role: 'Setter', badge: 'badge-gray', desc: 'Inbox access only. Reviews and approves AI-generated replies for their assigned bot. No analytics or settings access.' },
]
...
export default function UserManagement() {
  ...
  const [inviteForm, setInviteForm] = useState({ email: '', name: '', role: defaultInviteRole, assigned_bot_id: '', permissions: [...DEFAULT_CLIENT_PERMISSIONS] })
  ...
  async function loadData() {
    ...
    let invitesQuery = supabase.from('invites').select('id, email, name, token, role, status, expires_at, created_at, assigned_bot_id, invited_by').eq('status', 'pending').order('created_at', { ascending: false })
    if (profile?.role === 'client') invitesQuery = invitesQuery.eq('invited_by', profile.id)

    let botsQuery = supabase.from('bots').select('id, name').order('name')
    if (profile?.role === 'client') botsQuery = botsQuery.eq('id', profile.assigned_bot_id)

    const [{ data: usersData }, { data: invitesData }, { data: botsData }] = await Promise.all([usersQuery, invitesQuery, botsQuery])
  ...
  async function sendInvite() {
    ...
    const assignedBot = profile?.role === 'client' ? profile.assigned_bot_id : inviteForm.assigned_bot_id
    if (inviteForm.role === 'client' && !assignedBot) { showToast('Please assign a bot', 'error'); return }
    try {
      const token = Math.random().toString(36).substring(2) + Date.now().toString(36)
      const { error } = await supabase.from('invites').insert({
        email: inviteForm.email.toLowerCase(), name: inviteForm.name, token, role: inviteForm.role,
        ...
        invited_by: profile.id, expires_at: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(), status: 'pending'
      })
      ...
      loadData()
    } catch (error) { showToast(error.message || 'Failed to create invite', 'error') }
  }

  async function saveUserPermissions() {
    const { error } = await supabase.from('profiles').update({ role: editingUser.role, permissions: isFullAccess(editingUser.role) ? null : editingUser.permissions, assigned_bot_id: editingUser.assigned_bot_id || null }).eq('id', editingUser.id)
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <button className="btn btn-ghost" onClick={() => setShowRoleGuide(r => !r)}>
            {showRoleGuide ? 'Hide Role Guide' : 'Role Guide'}
          </button>
      ...
      {showRoleGuide && (
        <div className="card">
          <div className="card-title">Role Permissions Breakdown</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
            {ROLE_GUIDE.map(r => (
              <div key={r.role} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '12px 14px', background: 'var(--surf2)', borderRadius: 'var(--rsm)', border: '1px solid var(--bdr)' }}>
                <span className={`badge ${r.badge}`} style={{ flexShrink: 0, marginTop: '1px' }}>{r.role}</span>
                <div style={{ fontSize: '.83rem', color: 'var(--tx2)', lineHeight: 1.6 }}>{r.desc}</div>
              </div>
            ))}
          </div>
          <div>
            <div style={{ fontSize: '.72rem', fontWeight: 700, color: 'var(--tx3)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: '10px' }}>Individual Permissions (Client role)</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '8px' }}>
              {ALL_PERMISSIONS.map(p => (
                <div key={p.key} style={{ padding: '10px 12px', background: 'var(--surf)', border: '1px solid var(--bdr)', borderRadius: 'var(--rsm)' }}>
                  <div style={{ fontSize: '.8rem', fontWeight: 600, color: 'var(--tx)' }}>{p.label}</div>
                  <div style={{ fontSize: '.73rem', color: 'var(--tx3)', marginTop: '3px', lineHeight: 1.5 }}>{p.description}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ACTIVE USERS */}
      <div className="card">
        <div className="card-title">{profile?.role === 'client' ? 'Your Setters' : `Active Users (${users.length})`}</div>
        {users.length === 0 ? (
          <div style={{ color: 'var(--tx3)', fontSize: '.84rem', padding: '20px 0' }}>
            {profile?.role === 'client' ? 'No setters yet. Invite one above.' : 'No users yet.'}
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {users.map(u => (
              <div key={u.id} style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '14px', borderRadius: 'var(--rsm)', border: '1px solid var(--bdr)', background: 'var(--surf2)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', flexShrink: 0, background: 'var(--acc)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.88rem', fontWeight: 700, color: '#1A1A1A' }}>
                    {(u.name || u.email || 'U').charAt(0).toUpperCase()}
                  </div>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ fontSize: '.88rem', fontWeight: 600, color: 'var(--tx)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.name || '—'}</div>
                    <div style={{ fontSize: '.75rem', color: 'var(--tx3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.email}</div>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                  <span className={`badge ${u.role === 'superadmin' || u.role === 'admin' ? 'badge-green' : u.role === 'client' ? 'badge-blue' : 'badge-gray'}`}>{u.role}</span>
                  {u.botName && <span style={{ fontSize: '.76rem', color: 'var(--tx2)', background: 'var(--surf)', border: '1px solid var(--bdr)', padding: '2px 8px', borderRadius: '6px' }}>{u.botName}</span>}
                  <span style={{ fontSize: '.75rem', color: 'var(--tx3)', marginLeft: 'auto' }}>{isFullAccess(u.role) ? 'Full access' : `${(u.permissions || []).length} of ${ALL_PERMISSIONS.length} permissions`}</span>
                </div>
                <div style={{ display: 'flex', gap: '6px' }}>
                  {isFullAccess(profile?.role) && <button className="btn btn-ghost btn-sm" onClick={() => setEditingUser({ ...u, permissions: u.permissions || [] })}>Edit</button>}
                  {canRemove(u) && <button className="btn btn-ghost btn-sm" style={{ color: 'var(--red)' }} onClick={() => removeUser(u)}>Remove</button>}
                </div>
              </div>
            ))}
          </div>
      ...
      {invites.length > 0 && (
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {invites.map(inv => (
              <div key={inv.id} style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '14px', borderRadius: 'var(--rsm)', border: '1px solid var(--ambbd)', background: 'var(--ambbg)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', flexShrink: 0, background: 'var(--amb)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.88rem', fontWeight: 700, color: '#fff' }}>
                    {(inv.name || inv.email || '?').charAt(0).toUpperCase()}
                  </div>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ fontSize: '.88rem', fontWeight: 600, color: 'var(--tx)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{inv.name || '—'}</div>
                    <div style={{ fontSize: '.75rem', color: 'var(--tx3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{inv.email}</div>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                  <span className={`badge ${isFullAccess(inv.role) ? 'badge-green' : inv.role === 'client' ? 'badge-blue' : 'badge-gray'}`}>{inv.role}</span>
                  {inv.botName && <span style={{ fontSize: '.76rem', color: 'var(--tx2)', background: 'var(--surf)', border: '1px solid var(--bdr)', padding: '2px 8px', borderRadius: '6px' }}>{inv.botName}</span>}
                  <span style={{ fontSize: '.74rem', color: 'var(--tx3)', marginLeft: 'auto' }}>Expires {new Date(inv.expires_at).toLocaleDateString()}</span>
                </div>
                <div><button className="btn btn-ghost btn-sm" style={{ color: 'var(--red)' }} onClick={() => cancelInvite(inv.id)}>Cancel invite</button></div>
              </div>
            ))}
          </div>
        </div>
      ...
      {showInviteModal && (
        <ModalOverlay onClose={() => setShowInviteModal(false)}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <FormField label="Name *"><input className="form-input" value={inviteForm.name} onChange={e => setInviteForm(f => ({ ...f, name: e.target.value }))} placeholder="Full name" /></FormField>
            <FormField label="Email *"><input className="form-input" type="email" value={inviteForm.email} onChange={e => setInviteForm(f => ({ ...f, email: e.target.value }))} placeholder="their@email.com" /></FormField>
            {availableRoles.length > 1 && (
              <FormField label="Role *">
                <select className="form-input" value={inviteForm.role} onChange={e => { const r = e.target.value; setInviteForm(f => ({ ...f, role: r, permissions: r === 'setter' ? DEFAULT_SETTER_PERMISSIONS : DEFAULT_CLIENT_PERMISSIONS })) }}>
            ...
            {!isFullAccess(inviteForm.role) && inviteForm.role !== 'setter' && (
              <PermissionCheckboxes permissions={inviteForm.permissions} onChange={perms => setInviteForm(f => ({ ...f, permissions: perms }))} />
            )}
        ...
        <ModalOverlay onClose={() => setEditingUser(null)}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            {!isFullAccess(editingUser.role) ? (
              <PermissionCheckboxes permissions={editingUser.permissions || []} onChange={perms => setEditingUser(u => ({ ...u, permissions: perms }))} />
            ) : <AdminNote />}
...
function PermissionCheckboxes({ permissions, onChange }) {
  const toggle = (key) => permissions.includes(key) ? onChange(permissions.filter(p => p !== key)) : onChange([...permissions, key])
  return (
